...
### Fixed (second design review)
- **Mobile campaign cards** — the Campaigns list and the dashboard's live campaigns render one card per campaign below `sm` (name and status chip, progress bar, calls and completion), linking to the same edit page as the table; `GET /campaigns` and the campaign detail carry `session_count` and `completed_session_count` from one grouped query so the cards show real completion.
- **Every control carries the focus ring and the 44px target** — `BUTTON_PRIMARY` and `BUTTON_SECONDARY` include `FOCUS_RING`, and the secondary primitive replaces sixteen hand-rolled outline buttons; the modal close button, mobile hamburger, wizard back link and footer buttons, target drag handle, audio-slot version actions, stats filter bar, embed copy buttons and the phone input all meet the rule; checkboxes and radios take the brand accent colour.
